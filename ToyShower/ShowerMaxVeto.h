#ifndef ShowerMaxVeto_h
#define ShowerMaxVeto_h

#include "ShowerBase.h"

struct ShowerMaxVeto : public Shower {

  ShowerMaxVeto(int seed) : Shower(seed) {}
  ...
  struct Antenna : public Shower::Antenna {
    Antenna(Shower* showerPtr, int indexi, int indexj, int indexSelf, int indexLeft, int indexRight, double Q2start) : 
      ...
      // Add it to the queue
      dynamic_cast<ShowerMaxVeto*>(showerPtr)->queue.push(Trial(trialId, indexSelf, Q2trial));
    }
    ...
    void generate_scale(double Q2start) {
      if (Iz == 0.) {
        ...
        Q2trial = Q2start*pow(showerPtr->rng(), 1./a/Iz);
