#ifndef ShowerVetoMax_h
#define ShowerVetoMax_h

#include "ShowerBase.h"

struct ShowerVetoMax : public Shower {

  ShowerVetoMax(int seed) : Shower(seed) {}
  struct Antenna : public Shower::Antenna {
    Antenna(Shower* showerPtr, int indexi, int indexj, int indexSelf, int indexLeft, int indexRight, double Q2start) : 
      ...
      // Add it to the queue
      dynamic_cast<ShowerVetoMax*>(showerPtr)->queue.push(Trial(trialId, indexSelf, Q2trial));
    }
